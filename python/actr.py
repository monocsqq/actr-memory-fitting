...
def start (host=None,port=None, connection=current_connection):
...
def connection (port=None): # python側からポートを指定して接続
...
def connection (port=None, connection=current_connection): # python側からポートを指定して接続

    if connection == None:
        s = start(port=port, connection=connection)
        if s :
            print(f"ACT-R connection has been started: port{port}.")
            return s
        else:
            print(f"Could not start ACT-R connection: port{port}.")
    else:
        return connection
...
# current_connection = connection() # mod
...
def reset (connection=current_connection):
    return connection.evaluate_single("reset")
...
def run (time, real_time=False, connection=current_connection):
    return connection.evaluate("run", time, real_time)
...
def remove_command(name, connection=current_connection):
    return connection.remove_command(name)
...
# user added
def get_base_level(name):
    return current_connection.evaluate_single("get-base-level",name)

# user added
def set_base_levels(name,value):
    return current_connection.evaluate_single("set-base-levels",(name,value))
...
def set_parameter_value(param,value, connection=current_connection):
    return connection.evaluate_single("set-parameter-value",param,value)
